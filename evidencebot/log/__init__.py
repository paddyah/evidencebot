...
config.log_location.setFormatter(formatter.EvidenceBotLogFormatter())
logging.setLoggerClass(logger.EvidenceBotLogger)

# We use a project-wide logger because of how we shim metadata into log statements
evidencebot_logger = cast(logger.EvidenceBotLogger, logging.getLogger("evidencebot"))
...
# Ensure the handler is properly flushed when EvidenceBot is killed
atexit.register(logging.shutdown)


def set_third_party_logging() -> None:
    """
    Enable logging on third-party packages that can't be overwritten with EvidenceBotLogger
    """
...
# Forward evidencebot_logger's logging methods as module-level functions
debug = evidencebot_logger.debug
info = evidencebot_logger.info
warning = evidencebot_logger.warning
error = evidencebot_logger.error
critical = evidencebot_logger.critical
log = evidencebot_logger.log
exception = evidencebot_logger.exception
